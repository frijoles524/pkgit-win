...
void add_repo(package_t *pkg) {
	...
	FILE *wfile = fopen(cfg.repos.data, "w");
	if (wfile) {
		fprintf(wfile,
			"%srepositories[\"%s\"] = { url = \"%s\" }\n",
			previous_repos, pkg->name.data, pkg->url.data
		);
		fclose(wfile);
	}
}
